...
sub create
{
	...
	my $rc = 1;
        ...
        $rc &&= $dbh->do( "GRANT ALL PRIVILEGES ON ".$dbh->quote_identifier( $dbname ).".* TO ".$dbh->quote_identifier( $dbuser )."\@".$dbh->quote("%")." IDENTIFIED BY ".$dbh->quote( $dbpass )." WITH GRANT OPTION" );
		
		$rc &&= $dbh->do( "FLUSH PRIVILEGES" );
